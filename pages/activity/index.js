...
export default function index() {
    ...
    useEffect(async () => {
        const resData = Cookies.get('role') != 'Administrator' ? await callService('POST', `${serViceUrl()}allusers/getLogId`, { token: Cookies.get('cookie'), }) : await callService('POST', `${serViceUrl()}admin/getLogAll`, { token: Cookies.get('cookie'), })
        console.log(resData.data.message)
        let i = 0;
        resData.data.message.forEach(element => {
            var date = new Date(resData.data.message[i].time);
            resData.data.message[i].time = date.getFullYear() + '-' + date.getMonth().toString().padStart(2, '0') + '-' + date.getDay().toString().padStart(2, '0') + ' ' + date.getHours().toString().padStart(2, '0') + ':' + date.getMinutes().toString().padStart(2, '0')
            i++
        });
