...
export default function index() {
                ...
                return (
                    <a onClick={async () => {
                        ...
                        if (Cookies.get('role') == 'HRadministrator') {
                            const resData = await callService('POST', `${serViceUrl()}hr/getOneApproveLeave`, { token: Cookies.get('cookie'), leaveId: record.leaveId.toString() })
                            ...
                            const resData = await callService('POST', `${serViceUrl()}supervisor/getOneStaffLeaveId`, { token: Cookies.get('cookie'), leaveId: record.leaveId.toString() })
                            console.log(resData.data.message)
                            setLeaveDetail({
                                leaveId: resData.data.message.leaveId,
                                name: resData.data.message.name,
    ...
    useEffect(async () => {
        let url = Cookies.get('role') != 'Supervisor' ? 'hr/getApproveLeave' : 'supervisor/getStaffLeave'
        console.log(url)
